...
public class JsonKit {
  ...
  /** 构建 Options，可选包含 dateFormat 和 prettyFormat。 当两者都不需要时返回共享的 baseOptions。 */
  ...
  /** 设置 json 字符串转换为 aifei-db 的 Row 时，是否将字段名由驼峰转换为下划线。 默认值为 true。 */
  ...
  /** 设置 json 字符串转换为 aifei-db 的 Model 时，是否当成 Row 去转换，而非调用 setter 方法转换。 默认值为 false。 */
  public JsonKit setReadModelAsRow(boolean enabled) {
    JsonString.defaultModelAsRow = enabled;
    return this;
  }

  /** 设置 aifei-db 的 Row 转换为 json 字符串时，是否将字段名由下划线转换为驼峰。 默认值为 true。 */
  public JsonKit setWriteRowFieldSnakeToCamel(boolean enabled) {
    JsonObject.defaultSnakeToCamel = enabled;
    return this;
  }

  /** 设置 json 字符串转换为 Java 对象时，Row 字段由下划线转驼峰之前是否将字段转成小写， 适用数据表字段大写的数据库，例如 Oracle。 */
  ...
  /**
   * 设置 aifei-db 的 Model 转换为 json 字符串时，是否当成 Row 去转换，也即取出内部的 Map data 数据转成 json，而非调用 getter 方法取值转换。用于
   * SQL 关联查询将关联表字段一同转换为 json。 默认值为 false。
   */
  ...
  /** 设置 json 字符串转换为 Java 对象时的日期格式。 注意：一般不建议配置，而是利用 snack4 自动探测。 */
  public JsonKit setReadDateFormat(String dateFormat) {
    JsonString.defaultDateFormat = dateFormat;
    return this;
  }

  /** 设置格式化、缩进 json 字符串，提升可读性。 注意：有较大性能损耗，仅用于开发调试，生产环境请勿开启。 */
